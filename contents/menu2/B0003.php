...
$now_url		= "B0003.php";
$write_url		= "B0003_write.php";
$excel_url		= "fn.excel.order2.php";
$page			= $_GET['page'];
...
if(empty($_POST['f_storage_date']) && empty($_POST['t_storage_date'])) {
	$f_storage_date = date('Y-m-d');
	$t_storage_date = date('Y-m-d');
}

$sch_type = "storage";

// 메인쿼리(sch_type = order :발주, storage:입고
...
<table width="100%" cellspacing="0" cellpadding="0" border="0" style="height:50px;">
	<tr>
		<td colspan="" valign="bottom"><font color="#08519C" style="size:11px;font-weight:bold"><b>봉투신청/입고 > 발주 입고 현황</b></font></td>
		<td></td>
		<td></td>
	</tr>
	<tr>
...
<table  width=100% border=0 cellspacing=1 cellpadding=2 bgcolor=#f2f2f2 style="border-color: red">
	<tr>
		<td>
			...
			입고기간 : <input type='text' name='f_storage_date' id="datepicker1"  size='15' value='<?=date('Y-m-d')?>' /> ~ <input type='text' name='t_storage_date' id="datepicker2"  size='15' value='<?=date('Y-m-d')?>' /> 
			제작업체 : <select name="factory" id="factory">
				<option value="" <?=$selected1?>>업체선택</option>
			...
			<input type="button" value="발주입고 검색" onclick="javascript:fn_submit();" class="button_50" />
